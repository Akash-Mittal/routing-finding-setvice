...
public class RouteController {
    ...
    public ResponseEntity<RouteResponse> get(@RequestHeader(value = "x-secret") String secret,
            @RequestBody RouteRequest routeRequest) throws InterruptedException, ExecutionException {

        if (authService.validate(secret)) {
            return new ResponseEntity<>(routingService.getWinner(routeRequest), HttpStatus.ACCEPTED);
        }
